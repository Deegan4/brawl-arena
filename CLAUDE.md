...
6. **Effects / Audio** — `spark()` pushes into shared `particles[]`. Audio is a lazy WebAudio synth (`audioInit()` is called from the Start button click to satisfy autoplay policy); `tone()` and `noise()` are fire-and-forget. Master gain is scaled by `settings.sfxVol` and live-updates via `applySfxVol()`.
...
9. **Draw** — Order matters: background → platforms → fighters → particles → flash overlay. Screen shake is applied via `ctx.translate(shx, shy)` around the whole world draw, inside a `save`/`restore`.
